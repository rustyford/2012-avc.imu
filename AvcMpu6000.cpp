...
byte AvcMpu6000::MPU6000_SPI_read(byte reg)
{
  byte dump;
  byte return_value;
...
void AvcMpu6000::MPU6000_SPI_write(byte reg, byte data)
{
  byte dump;
  digitalWrite(MPU6000_CHIP_SELECT_PIN, LOW);
...
void AvcMpu6000::update (int newData) {
  int byte_H;
  int byte_L;


  if (newData > 0)   // Wait until we have new data from MPU6000 (INT6 interrupt)
  { 
  // Read AccelX
